...
#Creating summary table (total homocites, and totalunsolved number by city ) 
homocide_unsolved_table <- WP %>%
  group_by(city_state) %>%
  summarize(
    total_homicides = n(),
...
print(homocide_unsolved_table, n=Inf)
```
### Homocides in Baltimore, MD
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Creating function to caclulate proporation using prop.test that will be applied to every city 
prop_test_city <- function(data) {
  prop.test(
    x = data$total_unsolved,
    n = data$total_homicides,
...
#Runing for Baltimore, MD
baltimore_data <- homocide_unsolved_table %>%
  filter(city_state == "Baltimore, MD") %>% 
  prop_test_city()

```

The proportion of unsolved homicides in Baltimore, MD is estimated to be `r round(baltimore_data$estimate, 3)`, with a 95% confidence interval ranging from `r round(baltimore_data$conf.low, 3)` to `r round(baltimore_data$conf.high, 3)`.

### Homicides In All Cities 
The proportion of unsolved homicides varies across cities, as shown in the plot below which displays the estimated proportion of unsolved homicides along with their 95% confidence intervals for each city in the dataset. The highest was Chicago and the lowest was Tulsa, Alabama. 

```{r, echo=FALSE, message=FALSE, warning=FALSE, results=FALSE, fig.width=7, fig.height=10}
#Making function with Map and Mapp 2 using purrr pacakge 
prop_city <- function(unsolved, total) {
  prop.test(
    x = unsolved,
    n = total,
    alternative = "two.sided",
...
city_probability <- homocide_unsolved_table %>% 
  mutate(results = map2(total_unsolved, total_homicides, prop_city)) %>%
  unnest(results)
